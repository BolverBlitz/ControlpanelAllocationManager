...
const GetNodeResources = function (NodeID) {
    return new Promise(function (resolve, reject) {
        if (typeof NodeID !== "number") { reject(new Error("NodeID must be a number")); }
        pterodactyl_db.getConnection(function (err, connection) {
            connection.query(`SELECT memory, memory_overallocate, disk, disk_overallocate FROM panel.nodes WHERE nodes.id = ${NodeID};`, function (err, rows, fields) {
                connection.release();
                if (err) {
                    reject(err);
                }
...
const DelProductNodeLink = function (NodeID, ProductID) {
    return new Promise(function (resolve, reject) {
        if (typeof NodeID !== "number") { reject(new Error("NodeID must be a number")); }
        controlpanel_db.getConnection(function (err, connection) {
            connection.query(`DELETE FROM dashboard.node_product WHERE node_id = '${NodeID}' AND product_id = '${ProductID}';`, function (err, rows, fields) {
                connection.release();
                if (err) {
                    reject(err);
                }
...
const AddProductNodeLink = function (NodeID, ProductID) {
    return new Promise(function (resolve, reject) {
        ...
        controlpanel_db.getConnection(function (err, connection) {
            connection.query(`INSERT INTO dashboard.node_product (node_id, product_id) VALUES (?)`, [values], function (err, rows, fields) {
                connection.release();
                if (err) {
                    reject(err);
                }
